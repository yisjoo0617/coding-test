...
function solution(video_len, pos, op_start, op_end, commands) {
    ...
    const zeroCheck = (value) => (value < 10 ? '0' + value : value)
    ...
    const secondsToTime = (sconds) => {
        const m = Math.floor(sconds / 60)
        const s = sconds % 60
        return zeroCheck(m) + ':' + zeroCheck(s)
    }
    
    const new_video_len = timeToSeconds(video_len)
    let new_pos = timeToSeconds(pos)
    const new_op_start = timeToSeconds(op_start)
    const new_op_end = timeToSeconds(op_end)
    
    commands.forEach((command) => {
        if (new_pos >= new_op_start && new_pos <= new_op_end) {
            new_pos = new_op_end
        }
